...
def instruction_html(rule):
    """instruction은 문장 리스트(신형) 또는 문자열(구형) 둘 다 온다."""
    steps = rule.get("instruction") or "배출 방법 데이터를 준비 중입니다."
    if isinstance(steps, str):
        return esc(steps)
    return "<br>".join(f"• {esc(step)}" for step in steps)
